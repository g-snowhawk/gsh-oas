...
class Response extends \Gsnowhawk\Oas\Fixedasset
{
    ...
    public function edit($readonly = false) : void
    {
        ...
        $this->setHtmlId($html_id);
        $this->setHtmlClass('fixedasset edit');
        $this->view->render($template_path);
    }

    public function pdf()
    {
        $tYear = intval($this->request->POST('nendo'));
        ...
        foreach ($results as $result) {
            ...
            $data['depreciate_price'] = parent::depreciate($result, $lda, $tYear + $shift, true);
            ...
            if ($diff === 0) {
                $diff = $this->acquire_price_total;
            }
            $this->price_onhand_total = $this->acquire_price_total - $diff;
    ...
    public function drawClosing(&$result, &$price_onhand, $year, $month = 12, $date = 31)
    {
        ...
        $ary = $this->pdfMappingRow($da);
        $lda = ($result['item'] === $this->lumpsum_depreciable_assets);
        $tp = parent::depreciate($result, $lda, $year, false);
        if ($tp > $price_onhand) {
            $data['depreciate_price'] = $price_onhand;
            $memValue = ($lda) ? 0 : parent::MEMVALUE;
    ...
    public function getWareki($date)
    {
        ...
        $n = date('n', $timestamp);
        $j = date('j', $timestamp);
        $year = intval(date('Ymd', $timestamp));
        ...
        return sprintf(Lang::translate($key), $y, $n, $j);
    }
    ...
    public function _depreciate($startYear, $endYear, $result)
    {
        $total = 0;
